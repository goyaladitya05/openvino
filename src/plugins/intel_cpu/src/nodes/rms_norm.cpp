...
struct RMSNorm::RMSNormExecutor : public RMSNorm::Executor {
    ...
    void execute(const std::vector<MemoryPtr>& inputs,
        ...
        // RMS without gamma (elementwise_affine=false) has no scale input, multiply by exact 1.0f instead
...
void RMSNorm::createPrimitive() {
    ...
    // no scale input (elementwise_affine=false): the executor feeds the kernel a scalar 1.0f
...
bool RMSNorm::isSupportedOperation(const std::shared_ptr<const ov::Node>& op, std::string& errorMessage) noexcept {
    try {
        ...
        if (rms) {
            ...
            // check scale (absent for RMS without gamma, i.e. elementwise_affine=false)
